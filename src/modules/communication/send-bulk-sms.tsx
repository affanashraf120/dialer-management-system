...
interface Props {
  ...
  loading: boolean;
  onSend: (data: SendBulkSMSType) => void;
}
...
const SendBulkSMS = ({ debt_statuses, onSend, loading }: Props) => {
  const { searchClient } = useHttpClient();
  const [open, setOpen] = useState(false);

  const form = useForm<z.infer<typeof SendBulkSMSSchema>>({
  ...
  const handleSendBulkRequest = async (e: SendBulkSMSType) => {
    await onSend(e);
    toggle();
  };
